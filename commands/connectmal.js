...
module.exports = {
    ...
    execute: async (interaction) => {
        connection.query(`SELECT * FROM codes WHERE userID = '${interaction.user.id}';`).then(res =>{
            if (res[0][0]) {
            ...
            interaction.reply(`Please go to this url: ${url}`).then(async () => {
                ...
                console.log(code)
                if (code == 'err') {
                    connection.query(`DELETE FROM codes WHERE userID = '${interaction.user.id}';`).then(res => {}).catch(err => {
                        if (err) {
                ...
                connection.query(`CREATE TABLE IF NOT EXISTS tokens (userID MEDIUMTEXT NOT NULL, accToken MEDIUMTEXT NOT NULL, reqToken MEDIUMTEXT NOT NULL);`)
                connection.query(`INSERT INTO tokens (userID, accToken, reqToken) VALUES ('${interaction.user.id}', '${accToken}', '${refToken}');`).then(res => {}).catch(err => {
                    if (err) {
                        console.log(err)
                        connection.query(`DELETE FROM codes WHERE userID = '${interaction.user.id}';`)
